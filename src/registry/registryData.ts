...
export const REGISTRY_COMPONENTS: ComponentRegistryItem[] = [
  ...
  },
  {
    ...
    usageSnippet: `import { ToastContainer } from "@99/ui"\n\nconst [toasts, setToasts] = useState([]);\n<ToastContainer toasts={toasts} onDismiss={removeToast} />`,
    codeSnippet: `export const Toast = ({ message, type, onClose }) => <div>...</div>;`,
    props: [
      { name: 'toasts', type: 'ToastItem[]', description: 'Queue to render, oldest first. Capped at 3.' },
      { name: 'onDismiss', type: '(id: string) => void', description: 'Dismiss handler receiving the toast id.' },
    ],
  },
  {
    id: 'banner',
  ...
  },
  {
    ...
    name: 'object-card',
    title: 'Object Card',
    description: "The kit\u2019s richest composite card: six layouts selected by record type, with completion, streak, progress and relation affordances.",
    category: 'Data Display',
    version: '1.0.0',
    dependencies: ['lucide-react', 'motion'],
    cliCommand: 'npx @99/ui add object-card',
    features: [
      'Six type-selected layouts (task, ritual, project, memory, journal, literature)',
      'Type vocabulary overridable per host via typeLabels',
      'Read-only when onUpdate is absent',
      'Localised (LTR/RTL) type labels out of the box',
    ],
    usageSnippet: `import { ObjectCard } from "@99/ui"\n\n<ObjectCard\n  object={{ id: '1', type: 'TASK', status: 'ACTIVE', title: 'Ship the kit', createdAt: '2026-01-01' }}\n  relatedCount={2}\n  onUpdate={(id, patch) => save(id, patch)}\n/>`,
    codeSnippet: `import { ObjectCard } from '@/components/ui/ObjectCard';\n\nexport default function Demo() {\n  return <ObjectCard object={record} relatedCount={2} onUpdate={save} />;\n}`,
    props: [
      { name: 'object', type: 'ObjectCardRecord', description: 'Structural record shape — any host object that looks like one.' },
      { name: 'relatedCount', type: 'number', default: '0', description: 'Number of related records. The card never queries this itself.' },
      { name: 'onUpdate', type: '(id, patch) => void | Promise<void>', description: 'Persist sink. Omit for a read-only card.' },
      { name: 'isRTL', type: 'boolean', default: 'false', description: 'Drives label locale.' },
      { name: 'typeLabels', type: 'Record<string, ObjectCardTypeBadge>', description: 'Extend or override the built-in type vocabulary.' },
    ],
  },
  {
    id: 'motion',
    ...
    description: "The kit\u2019s shared spring vocabulary. Every transition in UI99 resolves to one of these constants — motion is a governed axis, not per-component taste.",
    category: 'Layout & Navigation',
    version: '1.0.0',
    dependencies: ['motion', 'motion/react'],
    cliCommand: 'npx @99/ui add motion',
    features: [
      'Named spring constants per weight class (docs/POLICY.md §3.4)',
      'overlayMotion presets so every overlay enters identically',
      'Reveal for scroll-triggered entrance',
      'staggerY for list choreography',
    ],
    usageSnippet: `import { motion, springs, Reveal } from "@99/ui"\n\n<motion.div transition={springs.interactive} />\n<Reveal delay={60}>Content</Reveal>`,
    codeSnippet: `import { useChoreography } from '@/components/ui/motion';\n\nexport default function Demo() {\n  const c = useChoreography();\n  return <motion.div transition={c.interactive} />;\n}`,
    props: [
      { name: 'springs', type: 'SpringConfig', description: 'Named spring constants: interactive, entrance, exit, emphasis.' },
      { name: 'overlayMotion', type: 'MotionPreset', description: 'Shared enter/exit presets for every overlay.' },
      { name: 'delay', type: 'number', description: 'Reveal only — entrance offset in ms.' },
    ],
  },
  {
    id: 'density-switcher',
    ...
    description: "The control surface for the kit's density axis. Writes one attribute on <html>; every control height and rhythm token resolves against --density-scale.",
    category: 'Layout & Navigation',
    version: '1.0.0',
    dependencies: [],
    registryDependencies: ['segmented-control'],
    cliCommand: 'npx @99/ui add density-switcher',
    features: [
      'Three modes: compact / default / comfortable (Material + HIG parity)',
      'One attribute on <html> — no component internals are touched',
      'Persists to localStorage; attribute is the source of truth',
      'Uncontrolled by default, fully controlled via value/onChange',
    ],
    usageSnippet: `import { DensitySwitcher } from "@99/ui"\n\n<DensitySwitcher fullWidth />`,
    codeSnippet: `import { DensitySwitcher } from '@/components/ui/DensitySwitcher';\n\nexport default function Demo() {\n  return <DensitySwitcher fullWidth />;\n}`,
    props: [
      { name: 'value', type: 'Density', description: 'Controlled value. Omit to let the switcher own the state.' },
      { name: 'onChange', type: '(density: Density) => void', description: 'Called on every change, after the attribute and storage are written.' },
      { name: 'fullWidth', type: 'boolean', default: 'false', description: 'Stretch the control to its container.' },
    ],
  },
];
