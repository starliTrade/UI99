  ...
  return (
    <div ref={containerRef} className={`relative inline-block ${className}`}>
      ...
      >
        ...
        <span>{start}</span>
        <ArrowRight className="icon-xs text-(--text-secondary)" />
        <span>{end}</span>
      </button>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1.5 p-3 rounded-(--radius-control) bg-white dark:bg-(--bg-elevated) border border-zinc-200 dark:border-white/[0.06] shadow-2xl z-popover animate-in fade-in dur-fast space-y-3 min-w-[280px]">
          ...
          <div className="grid grid-cols-2 gap-2 type-caption font-mono">
            <div>
              <label className="type-micro text-(--text-secondary) block mb-1">Start Date</label>
              <input
                type="date"
                value={start}
                onChange={(e) => {
            ...
            <div>
              <label className="type-micro text-(--text-secondary) block mb-1">End Date</label>
              <input
                type="date"
                value={end}
                onChange={(e) => {
