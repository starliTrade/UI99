/**
 * SAFA — Living Hero (Living Atmosphere & Depth Signature) (Build 04)
...
export function LivingHero({ selectedDate, onOpenCapture }: LivingHeroProps) {
  ...
  const contextData = useMemo(() => {
    ...
    if (hour >= 5 && hour < 12) {
      ...
      return {
        greeting: { fa: 'صبح به‌خیر، صفای عزیز', en: 'Good morning, Safa' },
        microcopy: copy,
        icon: Sunrise,
      ...
      return {
        greeting: { fa: 'عصر به‌خیر، صفا جان', en: 'Good afternoon, Safa' },
        microcopy: copy,
        icon: Sun,
      ...
      return {
        greeting: { fa: 'غروب به‌خیر، صفای عزیز', en: 'Good evening, Safa' },
        microcopy: copy,
        icon: Sunset,
      ...
      return {
        greeting: { fa: 'شب آرام، صفای جان', en: 'Peaceful night, Safa' },
        microcopy: copy,
        icon: Moon,
  ...
  return (
    <section className="relative w-full pt-2 pb-1 select-none">
      ...
      <div
        className={`absolute -top-10 left-1/2 -translate-x-1/2 w-full max-w-xl h-44 bg-gradient-to-b ${contextData.auraColor} rounded-full blur-3xl pointer-events-none opacity-80`}
      />

      {/* 2. Living Atmosphere Container */}
      <div className="relative p-5 sm:p-6 rounded-[28px] bg-[#0A0B10]/90 border border-white/[0.025] shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04),0_18px_40px_-10px_rgba(0,0,0,0.7)] backdrop-blur-xl overflow-hidden transition-all duration-300">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="space-y-1.5 min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 text-xs text-[#92929B] font-medium">
              <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-white/[0.03] border border-white/[0.035]">
          ...
          <div className="flex items-center gap-3.5 shrink-0 self-end md:self-center">
            ...
            <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-[#1A1A24] via-[#0E0E14] to-[#07070A] border border-white/[0.06] shadow-[inset_0_1px_1px_0_rgba(255,255,255,0.15),0_8px_24px_rgba(0,0,0,0.8)] flex items-center justify-center overflow-hidden shrink-0 group">
              
              {/* Subtle orbital ring animation */}
              <motion.div
                animate={prefersReducedMotion ? {} : { rotate: 360 }}
                transition={{ duration: 32, repeat: Infinity, ease: 'linear' }}
                className="absolute inset-1 rounded-full border border-dashed border-white/[0.08]"
              />
              ...
              <motion.div
                ...
                className="absolute w-2 h-2 rounded-full bg-rose-400/80 shadow-[0_0_10px_rgba(244,63,94,0.9)]"
              />
