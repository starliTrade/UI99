  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3 relative z-10">
        ...
        <span className="text-[10px] px-2.5 py-0.5 rounded-(--radius-pill) bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 font-medium">
          {season}
        </span>
      </div>
      ...
      <div className="space-y-2 relative z-10">
        ...
        <div className="pt-2 flex flex-wrap gap-1.5">
          {wishlist.map((item, idx) => (
            <span
              key={idx}
              className="text-[10px] px-2.5 py-0.5 rounded-(--radius-pill) bg-white/[0.04] text-[#EDEDEF] border border-white/[0.04]"
            >
              ✦ {item}
            </span>
          ))}
