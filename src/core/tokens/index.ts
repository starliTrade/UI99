...
/**
 * The radius law itself — `rounded`, `RadiusRung`, `SurfacePaddingStep` and
 * `radiusClassForPadding` — now lives in `src/lib/utils.ts` and is re-exported
 * here.
 *
 * It moved because it is *design-system geometry*, not application state. A
 * `Card` copied out of the registry with `npx @99/ui add card` has to keep
 * working in a project that has never heard of this app, and it was shipping an
 * import of `../../core/tokens` — a path that does not exist in the consumer's
 * tree. `lib/utils.ts` is the registry's own shared module, already depended on
 * by every copied component, so the dependency became one that resolves.
 *
 * Re-exported rather than moved-and-broken so the ~40 in-app call sites and the
 * test suite keep their existing import path. One definition, two doors.
 */
